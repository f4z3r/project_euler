...
def run(grid):
    ...
    max_val = 0
    for row in grid:
        row_max = get_array_max(row, 4)
        if row_max > max_val:
            max_val = row_max
    inverted_grid = zip(*grid)
    for col in inverted_grid:
        col_max = get_array_max(col, 4)
        if col_max > max_val:
            max_val = col_max
    for idx in range(20):
        ...
        if diag1_max > max_val:
            max_val = diag1_max
        if diag2_max > max_val:
            max_val = diag2_max
    return max_val

def get_array_max(array, length):
    if len(array) < length:
        return 0
    max = 0
    for idx in range(len(array) - length + 1):
        vals = array[idx:idx+length]
        prod = reduce(operator.mul, vals, 1)
        if prod > max:
            max = prod
    return max
